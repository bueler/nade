...
\begin{document}
\scriptsize \noindent Math 615 NADE (Bueler) \hfill \today \,(version 1)
\thispagestyle{empty}
...
It will cover Chapters 1, 2, 3, and 5 of the textbook,\footnote{R.~LeVeque, \emph{Finite Difference Methods \dots}, SIAM Press 2007}, plus sections 4.1 and 4.2 and some content from Appendices (see below), plus the ``Classical iterative methods'' slides (see the Daily Log tab).  On this Review Guide I list \emph{specific} material that will be covered.  Material different from what is listed below will \emph{not} be covered.  My goal is to only include topics that have appeared on homework and in lecture.
