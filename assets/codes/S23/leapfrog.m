...
% on advection equation
%     u_t + a u_x = 0
% for u(t,x), with periodic boundary conditions on [0,1]
% and initial condition u(x,0)=sin(6 pi x).  Uses
% m+1 grid points and a spatial grid with h = 1/(m+1).
% Forms the matrix A in the MOL system
%     U'(t) = A U(t).
...
A = - (a/(2.0*h)) * A;
% full(A)                      % check A (for m small)

% solve advection equation
Uold = u0(:);                  % user's initial condition, as col
U = Uold + (k/2) * A*Uold;     % first step is RK2, eqn (5.30)
U = Uold + k * A*U;
for n = 2:NN
    Unew = Uold + 2.0*k * A*U; % leapfrog is multi-step
    Uold = U;                  %   so save two vectors
    U = Unew;
end
